...
export const supabaseTerere = createClient(
  ...
  { auth: { storageKey: 'maraehub-terere-auth' } }
);
...
export const supabaseTineka = createClient(
  ...
  { auth: { storageKey: 'maraehub-tineka-auth' } }
);

export const supabaseWaioweka = process.env.REACT_APP_WAIOWEKA_ANON_KEY
  ? createClient('https://kifqftelvliywqkizsho.supabase.co', process.env.REACT_APP_WAIOWEKA_ANON_KEY, { auth: { storageKey: 'maraehub-waioweka-auth' } })
  : null;
