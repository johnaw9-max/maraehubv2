  ...
  return (
    <div className="panel" style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 10 }}>
      <div style={{
        ...
        background: isTrustee ? 'var(--brand)' : 'var(--info)',
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 12, color: 'var(--text3)' }}>
          {u.email} · Added {fmt(u.created_at)}
        </div>
      </div>
      <span style={{
        fontSize: 10, borderRadius: 20, padding: '2px 10px',
        fontWeight: 600, background: rs.bg, color: rs.color,
      }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        <button
          onClick={() => onRole(u.id, isTrustee ? 'community' : 'trustee')}
...
export default function UserManager() {
  ...
  async function handleUpdateRole(id, r) {
    await supabase.from('profiles').update({ role: r }).eq('id', id);
    fetchUsers();
  }
        ...
        <>
          <div style={{ marginBottom: 24 }}>
            ...
            {trustees.length === 0
              ...
              : trustees.map(u => <Card key={u.id} u={u} isTrustee={true} onEdit={openEdit} onRole={handleUpdateRole} onDelete={handleDelete} />)
            }
          </div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text2)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>
            ...
            {community.length === 0
              ...
              : community.map(u => <Card key={u.id} u={u} isTrustee={false} onEdit={openEdit} onRole={handleUpdateRole} onDelete={handleDelete} />)
            }
          </div>
        </>
      )}
