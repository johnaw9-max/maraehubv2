...
function Card({u,isTrustee,onEdit,onRole,onDelete}){
...
<button onClick={()=>onEdit(u)} style={{fontSize:11,color:'var(--brand)',background:'none',border:'1px solid var(--border)',borderRadius:6,padding:'4px 8px',cursor:'pointer'}}>Edit</button>
<button onClick={()=>onRole(u.id,isTrustee?'community':'trustee')} style={{fontSize:11,color:'var(--text2)',background:'none',border:'1px solid var(--border)',borderRadius:6,padding:'4px 8px',cursor:'pointer'}}>{isTrustee?'Make Community':'Make Trustee'}</button>
<button onClick={()=>onDelete(u.id,u.full_name)} style={{fontSize:11,color:'var(--danger)',background:'none',border:'1px solid var(--border)',borderRadius:6,padding:'4px 8px',cursor:'pointer'}}>Remove</button>
</div>
</div>
);}
...
{trustees.length===0?<div style={{fontSize:13,color:'var(--text3)',padding:'12px 0'}}>No trustees yet</div>:trustees.map(u=><Card key={u.id} u={u} isTrustee={true} onEdit={openEdit} onRole={handleUpdateRole} onDelete={handleDelete}/>)}
</div>
<div>
...
{community.length===0?<div style={{fontSize:13,color:'var(--text3)',padding:'12px 0'}}>No community members yet</div>:community.map(u=><Card key={u.id} u={u} isTrustee={false} onEdit={openEdit} onRole={handleUpdateRole} onDelete={handleDelete}/>)}
</div>
</>
)}
</div>
