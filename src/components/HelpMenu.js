  ...
  trustee: [
    ...
    },
    {
      ...
      items: [
        ...
        { q: 'Can I attach a document to an asset?', a: 'Yes — when adding or editing an asset, use "Link to Existing Document" to connect it to a warranty, manual, or purchase invoice already saved in your Document Library, instead of that evidence having nowhere to live. Once linked, the asset shows a "📄 Evidence" badge, and the document itself shows how many assets (and compliance items) it is evidence for back in the Documents list.' },
      ]
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        { q: 'What is "Link to Existing Expense"?', a: 'If a compliance renewal has a real cost behind it — a fire extinguisher servicing invoice, a BWOF inspection fee — you can link the compliance item to the actual expense already recorded in Finance, instead of the two living as separate, disconnected records. Once linked, the item shows a "💰 Cost" badge with the real amount, and the expense itself shows a matching "🆘 Linked" badge back to the compliance item in Finance — so either screen tells you the full picture, not just half of it.' },
    ...
    },
    {
      ...
      items: [
        ...
        { q: 'What do the "Evidence for N compliance items" and "Evidence for N assets" badges mean?', a: 'A document can be linked as evidence from either the Compliance tab or the Assets tab, using their own "Link to Existing Document" option — for example, one Emergency Preparedness Plan document can genuinely be evidence for several compliance items at once, or a warranty document can be linked to the asset it covers. When a document has one or more compliance items or assets linked to it, a separate badge for each appears showing how many, and hovering over either shows which ones. (Resolutions can also link to a document from Minutes, but that link doesn\'t yet show a reverse badge here — a known, logged gap, not a hidden one.)' },
      ]
    },
    {
    ...
    },
    {
      ...
      items: [
        ...
        { q: 'What is the Map section, and how do I use it?', a: [
          { text: 'A simple map of the marae\'s real location, with key emergency features marked — water sources, muster/evacuation points, and key assets. Set your marae\'s location once — either by typing its name or address into the search box, or, if search cannot find it, by right-clicking your location on Google Maps and entering the coordinates shown manually. Then use "+ Add Point" for each feature you want marked, giving it a type, a label, and its own coordinates.' },
          { text: 'Once a location is saved, a "View on Google Maps" link appears next to the coordinates, opening that exact point in Google Maps in a new tab.' },
          ...
          { text: 'The location search box and the map preview both only appear once a Google Maps API key has been configured for MaraeHub. If you see "Search not available" or "Map preview not available", the manual coordinate fields still work, and anything already saved is unaffected — the search and preview just are not connected yet.' },
        ] },
