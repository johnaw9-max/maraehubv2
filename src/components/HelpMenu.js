...
const HELP_TOPICS = {
  trustee: [
    ...
    },
    {
      ...
      icon: '🔄',
      items: [
        { q: 'What is the closed loop system?', a: 'The Task Board is connected to every other module in MaraeHub. When something becomes overdue — a compliance item, a project deadline, a service reminder, a meeting action, a strategic goal, or a grant deadline — MaraeHub automatically creates a High-priority task in the Task Board. This means overdue items from across the whole platform surface in one place, so no action falls through the cracks.' },
        { q: 'Which modules create tasks automatically?', a: 'Six modules feed into the Task Board automatically: Compliance Tracker (overdue items), Projects (overdue project due dates), Assets (overdue service reminders), Minutes (overdue meeting actions), Goals (goals past their target date), and Grants (deadlines arriving within 14 days). When you visit any of these tabs, MaraeHub checks for overdue items and creates tasks if they do not already exist.' },
        { q: 'What happens when I complete a task?', a: 'Completing a task in the Task Board feeds back to the source module automatically. Completing a compliance task marks the compliance item as done and advances its next due date. Completing a service reminder task advances the reminder date by its recurrence interval. Completing a meeting action task marks that action as Completed in the Minutes tab. Completing a project task moves the project to Review status. Completing a goal task moves the goal to In Progress.' },
        { q: 'What do the task title prefixes mean?', a: '"OVERDUE:" means a compliance item is past its due date. "PROJECT:" means a project deadline has been missed. "SERVICE:" means an asset service reminder is overdue. "ACTION:" means a meeting action is overdue. "GOAL:" means a strategic goal is behind schedule. "GRANT:" means a grant deadline is within 14 days. These prefixes identify the source so the system knows exactly which record to update when you complete the task.' },
        ...
        { q: 'Where can I see all open tasks across modules?', a: 'The Board View has an "Open Tasks — by Source" section that groups all active tasks by their origin module (Compliance, Projects, Asset Services, Meeting Actions, Strategic Goals, Grants, and Manual). Each group shows the task titles and due dates, and links directly to the Task Board. This gives trustees a full cross-platform picture without switching between tabs.' },
      ]
    },
    {
