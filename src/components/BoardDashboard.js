...
function Stars({ rating }) {
  ...
  return (
    <span style={{ color: '#f4a400', letterSpacing: 1 }}>
      {'★'.repeat(r)}{'☆'.repeat(5 - r)}
    </span>
  );
}
...
export default function BoardDashboard({ onNavigate }) {
  ...
  async function fetchAll() {
    ...
    const [bookRes, projRes, actRes, grantRes, remRes, assetRes, taskRes, feedRes, settingsRes] = await Promise.all([
      ...
      supabase.from('projects').select('id, name, status, progress, lead, due_date'),
  ...
  // KPIs
  const bookingsYTD     = d.bookings.filter(b => b.status === 'approved' && new Date(b.start_date) >= yearStart).length;
  const feedbackScores  = d.feedback.filter(f => f.rating_overall);
  const avgRating       = feedbackScores.length ? (feedbackScores.reduce((s, f) => s + f.rating_overall, 0) / feedbackScores.length) : null;
  const activeProjects  = d.projects.filter(p => p.status === 'active');
  const openActionsCount = d.actions.length;
  const approvedGrantsAmt = d.grants.filter(g => g.status === 'approved').reduce((s, g) => s + (g.amount || 0), 0);
  ...
  // Sections
  const upcomingBookings   = d.bookings.filter(b => b.status === 'approved' && b.start_date >= todayStr).slice(0, 5);
  ...
  const pipelineGrants     = d.grants.filter(g => !['approved', 'declined'].includes(g.status));
  ...
  const recentFeedback     = d.feedback.filter(f => f.experience).slice(0, 3);
  ...
  const KPI_TILES = [
    { label: 'Bookings YTD',      value: bookingsYTD,               icon: '📅', bg: '#e8eef8', color: '#1a4a8a' },
    { label: 'Avg Star Rating',   value: avgRating ? `${Number(avgRating).toFixed(1)} ★` : '—', icon: '⭐', bg: '#fdf8dc', color: '#7a5a00' },
    { label: 'Active Projects',   value: activeProjects.length,     icon: '📋', bg: '#e8f4ef', color: 'var(--brand)' },
    { label: 'Open Actions',      value: openActionsCount,          icon: '✅', bg: openActionsCount > 0 ? '#fdf0dc' : '#f5f5f5', color: openActionsCount > 0 ? 'var(--warning)' : 'var(--text3)' },
    { label: 'Grants Secured',    value: fmtMoney(approvedGrantsAmt), icon: '💰', bg: '#e8f4ef', color: 'var(--brand)' },
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        {/* ── UPCOMING BOOKINGS ──────────────────────────────────────── */}
        <div className="panel">
          <SectionTitle icon="📅" title="Upcoming Bookings" count={upcomingBookings.length} />
          {upcomingBookings.length === 0 ? (
            <div style={{ fontSize: 13, color: 'var(--text3)', fontStyle: 'italic' }}>No upcoming approved bookings</div>
        ...
        {/* ── ACTIVE PROJECTS ────────────────────────────────────────── */}
        <div className="panel">
          <SectionTitle icon="📋" title="Active Projects" count={activeProjects.length} />
          {activeProjects.length === 0 ? (
            <div style={{ fontSize: 13, color: 'var(--text3)', fontStyle: 'italic' }}>No active projects</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
        ...
        {/* ── GRANTS PIPELINE ────────────────────────────────────────── */}
        <div className="panel">
          <SectionTitle icon="💰" title="Grants Pipeline" count={pipelineGrants.length} />
          {pipelineGrants.length === 0 ? (
            <div style={{ fontSize: 13, color: 'var(--text3)', fontStyle: 'italic' }}>No active grant applications</div>
      ...
      <div className="panel" style={{ marginBottom: 8 }}>
        <SectionTitle icon="⭐" title="Community Feedback" />
          ...
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            {avgRating ? (
              <>
                ...
                <div style={{ fontSize: 22, color: '#f4a400', marginBottom: 6 }}>
                  <Stars rating={Number(avgRating)} />
                </div>
                <div style={{ fontSize: 12, color: 'var(--text3)' }}>from {feedbackScores.length} response{feedbackScores.length !== 1 ? 's' : ''}</div>
              </>
            ) : (
              <div style={{ fontSize: 13, color: 'var(--text3)', fontStyle: 'italic', paddingTop: 16 }}>No feedback yet</div>
            )}
          </div>
          <div>
            {recentFeedback.length === 0 ? (
              <div style={{ fontSize: 13, color: 'var(--text3)', fontStyle: 'italic' }}>No comments received yet</div>
