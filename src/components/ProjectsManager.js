  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <div style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.3, flex: 1, minWidth: 0, paddingRight: 8 }}>
          {p.name}
          {overdue && <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--danger)', background: '#faeae7', borderRadius: 4, padding: '2px 7px', marginLeft: 6, verticalAlign: 'middle' }}>OVERDUE</span>}
        </div>
        <StatusPill
          status={p.status}
      ...
      <div style={{ fontSize: 14, color: 'var(--text3)', marginBottom: 8, display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <div style={{ marginBottom: 4 }}>
        ...
        <div style={{ fontSize: 14, color: 'var(--text3)', marginTop: 3 }}>{p.progress || 0}%{subtaskCount > 0 ? ` · ${subtaskCount} subtask${subtaskCount !== 1 ? 's' : ''}` : ''}</div>
      </div>
      ...
      >
        ...
        <div style={{ flex: 1 }} />
        <button onClick={() => onEdit(p)} style={{ fontSize: 14, color: 'var(--brand)', background: 'none', border: '1px solid var(--border)', borderRadius: 5, padding: '4px 10px', cursor: 'pointer' }}>Edit</button>
        <button onClick={() => onDelete(p.id)} style={{ fontSize: 14, color: 'var(--danger)', background: 'none', border: '1px solid var(--border)', borderRadius: 5, padding: '4px 10px', cursor: 'pointer' }}>Delete</button>
      </div>
    </div>
  );
}
          ...
          <div key={status}>
            <div style={{ background: '#1a4a3a', color: '#fff', padding: '10px 14px', borderRadius: '10px 10px 0 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <span style={{ fontFamily: 'Playfair Display, serif', fontSize: 14, fontWeight: 600 }}>{meta.label}</span>
              </div>
            ...
            <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderTop: 'none', borderRadius: '0 0 10px 10px', padding: 10, minHeight: 120 }}>
              {col.length === 0 ? (
                <div style={{ fontSize: 14, color: 'var(--text3)', textAlign: 'center', padding: '20px 8px', fontStyle: 'italic' }}>No projects</div>
              ) : col.map(p => (
      ...
      {projects.map(p => {
        ...
        return (
          <div key={p.id} className="panel" style={{ borderLeft: overdue ? '3px solid var(--danger)' : undefined, cursor: 'pointer' }} onClick={() => openProject(p)}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 4 }}>
                  {p.name}
                  {overdue && <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--danger)', background: '#faeae7', borderRadius: 4, padding: '2px 8px', marginLeft: 8 }}>OVERDUE</span>}
                </div>
                <div style={{ display: 'flex', gap: 12, fontSize: 14, color: 'var(--text3)' }}>
                  {p.lead && <span>👤 {p.lead}</span>}
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }} onClick={e => e.stopPropagation()}>
                ...
                />
                <button onClick={() => openEdit(p)} style={{ fontSize: 14, color: 'var(--brand-light)', background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '5px 12px', cursor: 'pointer' }}>Edit</button>
                <button onClick={() => handleDelete(p.id)} style={{ fontSize: 14, color: 'var(--danger)', background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '5px 12px', cursor: 'pointer' }}>Delete</button>
              </div>
            </div>
            ...
            <div style={{ fontSize: 14, color: 'var(--text3)' }}>{p.progress}% complete</div>
            {p.notes && <div style={{ fontSize: 14, color: 'var(--text2)', marginTop: 8, fontStyle: 'italic' }}>{p.notes}</div>}
          </div>
        );
      })}
    </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: '1px solid var(--border)', borderRadius: 8, padding: '8px 15px', cursor: 'pointer', fontSize: 14, color: 'var(--text2)', fontFamily: 'DM Sans, sans-serif' }}
        >
          ← Back to Projects
        </button>
        <button className="btn-primary" style={{ fontSize: 14 }} onClick={() => openEdit(selectedProject)}>Edit Project</button>
      </div>
      ...
      <div className="panel" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 16, fontSize: 14, color: 'var(--text3)', flexWrap: 'wrap' }}>
        ...
        {selectedProject.notes && (
          <div style={{ fontSize: 14, color: 'var(--text2)', fontStyle: 'italic', marginBottom: 12, padding: '10px 12px', background: 'var(--surface2)', borderRadius: 8 }}>
            {selectedProject.notes}
          </div>
        )}
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: 'var(--text3)', marginBottom: 6 }}>
      ...
      <div className="panel">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div style={{ fontFamily: 'Playfair Display, serif', fontSize: 16, fontWeight: 600 }}>
            Subtasks
            {subtasks.length > 0 && (
              <span style={{ fontSize: 14, fontFamily: 'DM Sans, sans-serif', fontWeight: 400, color: 'var(--text3)', marginLeft: 8 }}>
        ...
        {subtasks.length === 0 ? (
          <div style={{ color: 'var(--text3)', fontSize: 14, padding: '8px 0 16px', fontStyle: 'italic' }}>No subtasks yet — add one below.</div>
        ) : (
          <div style={{ marginBottom: 16 }}>
            {/* Column headers */}
            <div style={{ display: 'grid', gridTemplateColumns: '28px 1fr 125px 110px 95px 32px', gap: 8, padding: '0 0 6px', borderBottom: '1px solid var(--border)', marginBottom: 6 }}>
              {['', 'Task', 'Assignee', 'Due Date', 'Priority', ''].map((h, i) => (
                <div key={i} style={{ fontSize: 14, fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</div>
              ))}
            </div>
            ...
            {subtasks.map(st => {
              ...
              return (
                <div key={st.id} style={{ display: 'grid', gridTemplateColumns: '28px 1fr 125px 110px 95px 32px', gap: 8, alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--cream2)' }}>
                  {/* Checkbox */}
                  <button
                    type="button"
                  ...
                  {/* Title */}
                  <div style={{ fontSize: 14, color: done ? 'var(--text3)' : 'var(--text1)', textDecoration: done ? 'line-through' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {st.title}
                  </div>

                  {/* Assignee */}
                  <div style={{ fontSize: 14, color: 'var(--text3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {st.assigned_to || '—'}
                  </div>

                  {/* Due date */}
                  <div style={{ fontSize: 14, color: 'var(--text3)' }}>{fmt(st.due_date)}</div>

                  {/* Priority badge */}
                  <span style={{ fontSize: 14, background: ps.bg, color: ps.color, borderRadius: 20, padding: '3px 10px', fontWeight: 600, whiteSpace: 'nowrap' }}>
                    {st.priority}
                  </span>
        ...
        <div style={{ borderTop: subtasks.length > 0 ? '1px solid var(--border)' : 'none', paddingTop: subtasks.length > 0 ? 16 : 0 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text2)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Add Subtask</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 160px 130px 110px', gap: 8, marginBottom: 8 }}>
            <input
              ...
              style={{ fontSize: 14 }}
            />
            <select className="form-input" value={subtaskForm.assigned_to} onChange={e => setSubtaskForm(f => ({ ...f, assigned_to: e.target.value }))} style={{ fontSize: 14 }}>
            ...
            <input type="date" className="form-input" value={subtaskForm.due_date} onChange={e => setSubtaskForm(f => ({ ...f, due_date: e.target.value }))} style={{ fontSize: 14 }} />
            <select className="form-input" value={subtaskForm.priority} onChange={e => setSubtaskForm(f => ({ ...f, priority: e.target.value }))} style={{ fontSize: 14 }}>
              {SUBTASK_PRIORITIES.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <select className="form-input" value={subtaskForm.status} onChange={e => setSubtaskForm(f => ({ ...f, status: e.target.value }))} style={{ fontSize: 14, width: 140 }}>
              {SUBTASK_STATUSES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            <button className="btn-primary" onClick={handleSaveSubtask} disabled={savingSubtask} style={{ fontSize: 14, padding: '9px 20px' }}>
